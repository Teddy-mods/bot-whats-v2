...
* teddy aqui ne carai (kaue) BOSQ
* teddy kaue  SCRIPT INI BOSQ
* teddy odin  kaue maciel!!!
* ARIS187 ID
* teddy gostoso kkk!!!
*/
...
const arrayBulan = ['Janeiroi', 'fevereiro', 'março', 'abril', 'Maio', 'Junho', 'Julho', 'agosto', 'setembro', 'Outubro', 'novembro', 'dezembro']
...
const config = {
    A187: '📍TEDDYbot📍',
    instagram: 'OFF',
    nomer: 'wa.me/12996398908',
    aktif: '24 JAM',
    youtube: 'https://youtube.com/channel/UCdACtP09kfp97TWm8HgoSMw',
    whatsapp: 'o kaue (teddy) ta com preguiça de fazer esse comando kk',
    tanggal: `TANGGAL: ${moment().format('DD')} ${bulan} ${moment().format('YYYY')}`,
    waktu: time
}

const { A187, encontro, Tempo, instagram, whatsapp, youtube, número, ativo, na hora } = config
   ...
   switch (prefix) {
      ...
      case 'info':
       ...
       case 'nulis':
           nulis(value)
               .then(data => {
                   client.sendMessage(id, '[ESPERE UM POUCO AI...', MessageType.text)
       ...
       case 'ytmp3':
           ytdl('mp3', value)
               .then(data => {
                   ...
                   client.sendMessage(id, '[ESPERE UM POUCO ANIMAL...', MessageType.text)
       ...
       case 'ytmp4':
           ytdl('mp4', value)
               .then(data => {
                   const { judul, size, hasil: link } = data
                   let hasil = `OLA AQUI ESTA O LINK DE DOWNLOAD DA MSC\nCLIQUE NO LINK ABAIXO:\nMUSICA: ${judul}\n\nTamanho Do audio: ${size}\n\nLink: ${link}`
                   client.sendMessage(id, '[ESPERE UM BOT TA COM PREGUIÇA...', MessageType.text)
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
       ...
       case 'covid':
           corona()
               .then(data => {
                   ...
                   let hasil = `📌ÚLTIMOS DADOS DO DISTRITO COVID-19 DA brasil\n\n📍Positivo ==> ${positif} \n📍Curado ==> ${sembuh} \n📍Morreu ==> ${meninggal}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
       ...
       case 'pict':
           switch (value) {
               case 'cewek':
                   cewePict()
                       .then(buffer => {
                           client.sendMessage(id, '[ESPERE SO ESPERE KK...', MessageType.text)
               ...
               case 'cowok':
                   cowoPict()
                       .then(buffer => {
                           client.sendMessage(id, '[ESPERE BOT NA PUNHETA ...', MessageType.text)
                           client.sendMessage(id, buffer, MessageType.image)
                       })
                       .catch(err => {
                           console.log(err)
